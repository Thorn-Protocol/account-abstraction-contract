...
describe("Modular Smart Account Basics", async () => {
    ...
    it("Can send an ERC20 Transfer userOp", async () => {
        ...
        const userOp = await makeEcdsaModuleUserOp(
            "execute_ncC",
            [mockToken.address, ethers.utils.parseEther("0"), encodeTransfer(deployer.address, tokenAmountToTransfer.toString())],
            userSA.address,
            deployer,
            entryPoint,
            ecdsaModule.address,
        ...
        const tx = await entryPoint.connect(deployer).handleOps([userOp], beneficiaryAddress);
        console.log("Tx = ", tx);
    ...
    it("Can send a Native Token Transfer userOp", async () => {
        ...
        // const userOp = await makeEcdsaModuleUserOp("execute_ncC", [charlie.address, amountToTransfer, "0x"], userSA.address, smartAccountOwner, entryPoint, ecdsaModule.address, {
        //     preVerificationGas: 50000,
        // });
        // await environment.sendUserOperation(userOp, entryPoint.address);
